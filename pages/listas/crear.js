import { Container, Grid, TextField, makeStyles, Typography, Button, LinearProgress } from '@material-ui/core';
import { createRef, useState } from 'react';
import { MuiPickersUtilsProvider, KeyboardDatePicker } from '@material-ui/pickers';
import DateFnsUtils from '@date-io/date-fns';
import axios from 'axios';
import { parseCookies } from 'nookies';
import { COOKIES_USER_ID } from '../../utils/consts';
...
function Crear() {
    ...
    const createOrder = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post('/api/list', { ...form, userId, date: getDateFormatted(form.date) });
            setErrors(false);
            setSuccess(true);
            router.push(`/listas/${data.date}`).then(() => setShowSpinner(false));
        } catch (error) {
            setErrors(true);
            setErrorMessage(
